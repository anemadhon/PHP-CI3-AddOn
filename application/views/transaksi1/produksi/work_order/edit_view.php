	...
	<?php  $this->load->view("_template/nav.php")?>
										...
										<fieldset>
											...
											<div class="form-group row">
												...
												<div class="col-lg-9">
													...
													<input type="hidden" value="<?= $wo_header['kode_paket']?>" id="kode_paket" nama="kode_paket" readonly="">
													<input type="hidden" id="wolocked" readonly="">
												</div>
											</div>
											...
											<div class="form-group row">
												<label class="col-lg-3 col-form-label">Outsanding Qty To Integrate</label>
												<div class="col-lg-9">
													<input type="text" class="form-control" readonly="" value="<?= number_format($wo_header['outstd_qty'],4,'.','')?>" id="outstd_qty" nama="outstd_qty">
												</div>
											</div>
								...
								<table id="table-manajemen" class="table table-striped " style="width:100%">
									<thead>
										<tr>
											...
											<th>Uom</th>
											<th>On Hand</th>
											<th>Outsanding Qty To Integrated</th>
										</tr>
									</thead>
									<tbody>
									</tbody>
		...
		<script>
        	...
        	$(document).ready(function(){
				...
				let qty_paket = $('#outstd_qty').val(); //qty_paket
				let stts = $('#status').val();
				let Quantity = '';
				...
				$.post("<?php echo site_url('transaksi1/wo/wo_header_uom');?>",{material_no: kode_paket},(data)=>{
					...
					$('#table-manajemen').DataTable({
						...
						"initComplete": function(settings, json) {
							...
							$("#table-manajemen > tbody").find("tr:odd").hide()

							$("#table-manajemen > tbody").find("tr").each(function()
							{
								let td = $(this).find('td')
						...
						"columns": [
							...
							{"data":"OutQtyIntg", "className":"dt-center"}
						]
					});
				});
				...
				checkcheckbox = () => {
					let tbody = $("#table-manajemen tbody");
				...
				tbody.on('change','#descmat', function(){
					...
					tableExpand = document.getElementById("table-manajemen").rows[noEven].cells;
			...
			function addDatadb(id_approve = ''){
				...
				table.find('tr:even').each(function(i, el){
					...
					matrialDesc.push(td.eq(4).find('select option:selected').text().trim());
