											...
											<div class="form-group row">
												...
												<div class="col-lg-9">
													...
													name="srEntry" id="srEntry" onchange="getDataHeader(this.value)">
														<option value="123">Select Item</option>
		...
		<script>
			...
			function getDataHeader(srNumber){
				...
				$.post("<?php echo site_url('transaksi1/Transferoutinteroutlet/getHeaderTransferOut');?>",{srNumberHeader: srNumber},(data)=>{
					...
					cboMatrialGroup.html('<option value="123">Select Item</option><option value="all">All</option>');

					$.each(value.dataOption,(val, text)=>{
						$(`<option value="${text}">${text}</option>`).appendTo(cboMatrialGroup);
					})
					$('#srEntry').prop('disabled', 'disabled')

					cboMatrialGroup.change(()=>{

						showMatrialDetailData(cboMatrialGroup.val(),value.data.VBELN, (dataDet)=> {
							if(dataDet.length > 0) {
								$('#MatrialGroup').prop('disabled', 'disabled')
								// var srEntryOpt = $('#srEntry  option:selected').val()
								// console.log("SR ENTRY : ", srEntryOpt)
								// if(srEntryOpt === '123') {
								// 	$('#srEntry').removeAttr('disabled')
								// 	$('#MatrialGroup').removeAttr('disabled')
								// }
			...
			function showMatrialDetailData(cboMatrialGroup='',do_no='', cb){
